...
var
  a, b: real;
begin
  for var i := Length(symbol) downto 1 do
  begin
    if (i - 1 >= 1) and (symbol[i] in '+-*/') and (symbol[i - 1] in '+-*/') and (head^.next <> nil) then begin
      a := head^.data;
      kill_head(head);
      b := head^.data;
      kill_head(head);
      List(calc(a, b, symbol[i]), head);
      var q := i - 1;
      while head^.next <> nil do
      begin
        if q >= 1 then List(calc(head^.data, head^.next^.data, symbol[q]), head);
        kill_element(2, head);
        kill_element(2, head);
        dec(q);
      end;
